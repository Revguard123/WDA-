...
// Default alert recipient. Every ops alert goes here unless ALERT_EMAIL is set to
// override it. Hardcoded so alerting works even if the env var is never set.
export const DEFAULT_ALERT_TO = 'James@intergovsolutions.com';
...
export function renderAlertHTML({ subject, summary, rows = [] }) {
  ...
  return `<div style="background:#f8f4ec;padding:24px;font-family:${BODY};">
    <div style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e5e0da;border-top:4px solid #f52ea9;border-radius:10px;padding:24px 26px;">
      <div style="font-size:12px;letter-spacing:1.6px;text-transform:uppercase;color:#c05f0e;font-weight:600;">War Dogs Academy &middot; Ops alert</div>
      <div style="font-size:20px;font-weight:600;color:#1a1a1a;margin-top:8px;">${esc(subject)}</div>
...
export async function sendOpsAlert({ subject, summary, rows = [], to } = {}, { client } = {}) {
  const recipient = to || process.env.ALERT_EMAIL || DEFAULT_ALERT_TO;
  ...
  try {
    const data = await sendBatchEmail({ to: recipient, from, subject: `[WDA alert] ${subject}`, html }, { client });
