...
// Look up a buyer by email (powers the magic-link re-entry page). Returns the
// buyer row or null. Case-insensitive on the stored email.
export async function getBuyerByEmail(email) {
  ...
  const { data, error } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (error) throw new Error(`buyer email lookup failed: ${error.message}`);
  return data || null;
}
...
export async function incrementBatchesSent(buyerId, { expectedBatchesSent } = {}) {
  const supabase = await getServiceClient();
  const { data, error } = await supabase
    .from('buyers')
    .select('batches_sent, batches_owed, status')
    .eq('id', buyerId)
    .single();
  if (error) throw new Error(`buyer read failed: ${error.message}`);
  if (expectedBatchesSent != null && Number(data.batches_sent || 0) !== Number(expectedBatchesSent)) {
    return { batches_sent: data.batches_sent || 0, status: data.status };
  }
  ...
  let q = supabase
    ...
    .eq('id', buyerId);
  if (expectedBatchesSent != null) q = q.eq('batches_sent', expectedBatchesSent);
  const { data: updated, error: updErr } = await q.select('batches_sent, status').maybeSingle();
  if (updErr) throw new Error(`buyer update failed: ${updErr.message}`);
  return updated || { batches_sent: data.batches_sent || 0, status: data.status };
}
