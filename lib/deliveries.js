...
export function partitionRepeats(items, delivered = {}) {
  ...
  const seenSoliRaw = delivered.solicitations instanceof Set ? delivered.solicitations : new Set(delivered.solicitations || []);
  const seenSoli = new Set([...seenSoliRaw].map(normalizeSolicitationNum).filter(Boolean));
  const fresh = [];
  const repeats = [];
  const seenInBatch = new Set();
  for (const item of items) {
    const soli = normalizeSolicitationNum(item.solicitation_num);
    const identity = procurementIdentity(item);
    if (seenNotice.has(item.notice_id) || (soli && seenSoli.has(soli)) || (identity && seenInBatch.has(identity))) {
      repeats.push(item);
    } else {
      if (identity) seenInBatch.add(identity);
      fresh.push(item);
    }
  }
...
export async function getDeliveredKeys(buyerId, client) {
  ...
  if (noticeIds.size) {
    ...
    for (const o of opps || []) {
      const normalized = normalizeSolicitationNum(o.solicitation_num);
      if (normalized) solicitations.add(normalized);
    }
  }
  return { noticeIds, solicitations };
}

export async function countDeliveriesForBuyer(buyerId) {
  const supabase = await getServiceClient();
  const { count, error } = await supabase
    .from('deliveries')
    .select('notice_id', { count: 'exact', head: true })
    .eq('buyer_id', buyerId);
  if (error) throw new Error(`deliveries count failed: ${error.message}`);
  return count || 0;
}
...
export async function persistBatch({ buyerId, batchMonth, opportunityRows, items }) {
  ...
  const identities = fresh.map(procurementIdentity).filter(Boolean);
  if (identities.length !== new Set(identities).size) {
    throw new Error('FINAL PROCUREMENT DEDUPE ASSERTION FAILED: selected batch contains duplicate solicitation identities.');
  }
