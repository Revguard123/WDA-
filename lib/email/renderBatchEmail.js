...
function chip(label, value) {
  return (
    ...
    `font-family:${BODY};">${esc(label)}: <strong style="color:${BRAND.text};font-weight:600;">${esc(value)}</strong></span>`
  );
}
...
function statTile(num, label, accent, first) {
  return (
    ...
    `<div class="wda-statnum" style="font-family:${DISPLAY};font-size:34px;line-height:1;font-weight:600;color:${accent || BRAND.ink};">${esc(num)}</div>` +
    `<div style="font-family:${BODY};font-size:11px;letter-spacing:1px;text-transform:uppercase;color:${BRAND.muted};margin-top:8px;font-weight:600;">${esc(label)}</div>` +
    `</td>`
  );
}
...
function funnelStrip(stats, count) {
  ...
  <tr><td class="wda-pad" style="background:${BRAND.card};border-left:1px solid ${BRAND.line};border-right:1px solid ${BRAND.line};padding:4px 26px 24px 26px;">
    <div style="font-family:${BODY};font-size:12px;letter-spacing:1.6px;text-transform:uppercase;color:${BRAND.muted};font-weight:600;margin-bottom:12px;">How we built your brief</div>
...
function card(c, links, idx, now) {
  ...
  const pill = top
    ? `<span style="display:inline-block;background:${BRAND.pink};color:#ffffff;font-family:${BODY};font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;padding:6px 12px;border-radius:20px;">Top match</span>`
    : `<span style="display:inline-block;background:#fff3e6;color:${BRAND.orangeDeep};font-family:${BODY};font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;padding:6px 12px;border-radius:20px;">Strong fit</span>`;
  ...
  <tr><td class="wda-card" style="padding:0 0 22px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.card};border:1px solid ${BRAND.line};border-radius:12px;box-shadow:0 2px 8px rgba(26,26,26,0.05);">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
          <td valign="middle" style="width:48px;">
            <div style="width:42px;height:42px;background:${badgeColor};border-radius:10px;text-align:center;">
              <span style="font-family:${DISPLAY};font-size:19px;font-weight:600;color:#ffffff;line-height:42px;">${rank}</span>
            </div>
          </td>
      ...
      <tr><td class="wda-cardpad" style="padding:14px 24px 4px 24px;">
        <div style="font-family:${DISPLAY};font-size:22px;font-weight:600;color:${BRAND.ink};line-height:1.22;letter-spacing:-0.3px;">${esc(c.title || 'Untitled solicitation')}</div>
      ...
      <tr><td class="wda-cardpad" style="padding:4px 24px 0 24px;">
        <div style="display:inline-block;background:#fff3e6;border:1px solid #ffd9b0;border-radius:5px;padding:6px 12px;font-family:${BODY};font-size:13px;font-weight:600;color:${BRAND.orangeDeep};">${dueText}</div>
      </td></tr>
      ${c.why_line ? `<tr><td class="wda-cardpad" style="padding:16px 24px 4px 24px;">
        <div style="background:${BRAND.paper};border-left:3px solid ${BRAND.orange};padding:14px 16px;border-radius:0 8px 8px 0;font-family:${BODY};font-size:15.5px;color:${BRAND.text};line-height:1.55;">
          <div style="color:${BRAND.orangeDeep};text-transform:uppercase;font-size:12px;letter-spacing:0.8px;font-weight:600;margin-bottom:5px;">Why this is winnable for you</div>${esc(c.why_line)}
        </div></td></tr>` : ''}
      <tr><td class="wda-cardpad" style="padding:18px 24px 24px 24px;">
        <a class="wda-btn" href="${esc(deepUrl)}" style="display:inline-block;background:${BRAND.ink};color:#ffffff;text-decoration:none;font-family:${BODY};font-size:15px;font-weight:600;padding:13px 24px;border-radius:8px;margin:0 8px 8px 0;">See Full Breakdown &rarr;</a>
...
function howItWorks() {
  const step = (n, t) =>
    `<td width="33.33%" valign="top" style="padding:0 8px;">
      <div style="font-family:${DISPLAY};font-size:17px;font-weight:600;color:${BRAND.pink};">${n}</div>
  ...
  <tr><td class="wda-pad" style="background:${BRAND.panel};border-left:1px solid ${BRAND.line};border-right:1px solid ${BRAND.line};padding:24px 24px 22px 24px;">
    <div style="font-family:${BODY};font-size:12px;letter-spacing:1.6px;text-transform:uppercase;color:${BRAND.muted};font-weight:600;margin-bottom:14px;">How the brief works</div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
  ...
  const widenedNote = meta.widened
    ? `<tr><td class="wda-pad" style="background:${BRAND.card};border-left:1px solid ${BRAND.line};border-right:1px solid ${BRAND.line};padding:0 26px 18px 26px;">
        <div style="background:#fff3e6;border:1px solid #ffd9b0;border-left:3px solid ${BRAND.orange};border-radius:0 8px 8px 0;padding:12px 15px;font-family:${BODY};font-size:14px;color:${BRAND.orangeDeep};line-height:1.5;">
          <strong style="font-weight:600;">Your niche ran a little light this cycle,</strong> so we widened the net (looser deadlines${meta.widened.tiers?.some((t) => t.tier === 'nationwide') ? ' and outside your home state' : ''}) to keep your brief full. Tighten or loosen anytime from your targeting page.
        </div></td></tr>`
    : '';
      ...
      <table role="presentation" class="wda-shell" width="100%" cellpadding="0" cellspacing="0" style="width:100%;max-width:640px;">
        ...
        <tr><td class="wda-pad" style="background:${BRAND.card};border-left:1px solid ${BRAND.line};border-right:1px solid ${BRAND.line};padding:32px 28px 24px 28px;" align="center">
          ${logoUrl
            ...
            : `<div style="font-family:${DISPLAY};font-size:34px;font-weight:600;color:${BRAND.pink};letter-spacing:0.5px;">WAR DOGS <span style="color:${BRAND.orange};">ACADEMY</span></div>`}
          <div style="height:3px;width:66px;background:${BRAND.pink};margin:20px auto 14px auto;font-size:0;line-height:0;">&nbsp;</div>
          <div style="font-family:${BODY};font-size:14px;letter-spacing:3px;text-transform:uppercase;color:${BRAND.ink};font-weight:600;">${esc(BRAND.product)}</div>
        ...
        <tr><td class="wda-pad" style="background:${BRAND.card};border-left:1px solid ${BRAND.line};border-right:1px solid ${BRAND.line};padding:8px 28px 22px 28px;">
          <div class="wda-h1" style="font-family:${DISPLAY};font-size:27px;font-weight:600;color:${BRAND.ink};margin-bottom:10px;letter-spacing:-0.4px;">${greetingName},</div>
        ...
        <tr><td class="wda-pad" style="background:${BRAND.paper};border-left:1px solid ${BRAND.line};border-right:1px solid ${BRAND.line};padding:24px 16px 4px 16px;">
          <div style="padding:0 6px 12px 6px;">
            <span style="font-family:${DISPLAY};font-size:16px;font-weight:600;color:${BRAND.ink};text-transform:uppercase;letter-spacing:0.5px;">Your Targets</span>
        ...
        <tr><td class="wda-pad" style="background:${BRAND.card};border:1px solid ${BRAND.line};border-top:none;border-radius:0 0 12px 12px;padding:24px 28px 28px 28px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
            <td>
              <a href="${esc(links.allContracts)}" style="display:inline-block;background:${BRAND.paper};border:1px solid ${BRAND.line};color:${BRAND.ink};font-family:${BODY};font-weight:600;text-decoration:none;font-size:14px;padding:11px 18px;border-radius:8px;margin:0 6px 8px 0;">See all your contracts</a>
              <a href="${esc(links.targeting)}" style="display:inline-block;background:${BRAND.paper};border:1px solid ${BRAND.line};color:${BRAND.ink};font-family:${BODY};font-weight:600;text-decoration:none;font-size:14px;padding:11px 18px;border-radius:8px;margin:0 6px 8px 0;">Update your targeting</a>
            </td>
          </tr></table>
