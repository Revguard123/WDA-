...
export function opportunityUpsertPayload(rows, fetchedAt = new Date().toISOString()) {
  return (rows || []).map((row) => Object.fromEntries(
    OPPORTUNITY_COLUMNS
      .filter((key) => key === 'fetched_at' || Object.hasOwn(row, key))
      .map((key) => [key, key === 'fetched_at' ? fetchedAt : row[key]]),
  ));
}
...
export async function upsertOpportunities(rows) {
  ...
  const payload = opportunityUpsertPayload(rows);
