...
// when the cache is cold or when we deliberately need to reach past it (the
// geography-widen tier). Descriptions are resolved later, for finalists only.
async function poolForNiche(niche, { now, forceLive = false } = {}) {
  if (!forceLive) {
    const cached = await readCachedOpportunities(niche, { now });
    if (cached.length >= MIN_CACHE_POOL) return { rows: cached, source: 'cache' };
  }
  const { rows } = await runEngineForNiche(niche, {
...
export async function runBatchForBuyer(buyer, options = {}) {
  ...
  for (const tier of ladder) {
    ...
    // The geography-widen tier must reach past the (state-scoped) cache, so it
    // pulls live; the base and runway tiers read the cache first.
    const { rows } = await poolForNiche(tier.niche, { now, forceLive: tier.label === 'nationwide' });
