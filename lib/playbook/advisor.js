...
function fallbackUpdate(answers, category, answer) {
  ...
  const fulfillmentFromSentence = ['fulfillment', 'fulfillment_clarification', 'operating_model'].includes(category) ? fulfillmentFromText(value) : null;
...
function deliveryDimensionsFromAnswer(category, answer) {
  const dimensions = [];
  if (['fulfillment', 'fulfillment_clarification', 'operating_model'].includes(category) && fulfillmentFromText(answer)) dimensions.push('fulfillment');
  if (['fulfillment', 'fulfillment_clarification'].includes(category) && operatingResolvedByDeliveryText(answer)) dimensions.push('operating_model');
  return dimensions;
}
function applyServerDerivedUpdates(answers, category, answer, logger = null) {
  const updates = {};
  const fulfillment = ['fulfillment', 'fulfillment_clarification', 'operating_model'].includes(category) ? fulfillmentFromText(answer) : '';
  ...
  if (resolved.length > 1) safeAdvisorLog(logger, 'advisor_multi_dimension_resolution', { resolved });
...
export async function advanceAdvisorConversation({ answers = {}, resolved_dimensions = [], latest_answer = '', answered_category = '', turn_count = 0, client, logger = console } = {}) {
  ...
  const incomingFulfillment = ['fulfillment', 'fulfillment_clarification', 'operating_model'].includes(canonicalAnsweredCategory) ? fulfillmentFromText(latest_answer) : '';
  ...
  try {
    ...
    const explicitFulfillmentCorrection = incomingFulfillment && previousFulfillment && previousFulfillment !== 'unknown' && incomingFulfillment !== previousFulfillment && explicitCorrection(latest_answer);
    const directResolved = ADVISOR_DIMENSIONS.includes(canonicalAnsweredCategory) && !(explicitFulfillmentCorrection && canonicalAnsweredCategory === 'operating_model') ? [canonicalAnsweredCategory] : [];
    const baseResolved = explicitFulfillmentCorrection ? priorResolved.filter((dimension) => dimension !== 'operating_model') : priorResolved;
    const serverResolvedSeed = [...baseResolved, ...derived.resolved_dimensions, ...directResolved];
    ...
    const nextResolved = [...new Set([...serverResolved, ...trustedClaudeResolved])].filter((dimension) => !(explicitFulfillmentCorrection && dimension === 'operating_model'));
