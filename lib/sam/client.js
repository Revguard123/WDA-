...
// naicsCode, typeOfSetAside and state are optional narrowing filters.
...
export async function samSearchPage(options, { fetchImpl = fetch } = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`SAM search failed (${res.status}): ${body.slice(0, 500)}`);
  }
  return res.json();
}
